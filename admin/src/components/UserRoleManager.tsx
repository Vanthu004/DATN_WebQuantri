  ...
  const handleRoleChange = async () => {
    ...
    try {
      ...
      if (response.data) {
        ...
        setMessage({ 
          type: 'success', 
          text: `Đã cập nhật role của ${user.name} thành ${selectedRole}` 
        });
        onRoleUpdate(responseData.user);
      }
    } catch (error: unknown) {
      const errorMessage = error instanceof Error ? error.message : 'Có lỗi xảy ra khi cập nhật role';
    ...
    <div className="bg-white rounded-lg shadow p-4">
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <select
            value={selectedRole}
            onChange={(e) => setSelectedRole(e.target.value as "admin" | "customer" | "user")}
        ...
        <div className="flex space-x-3">
          <button
            ...
            disabled={isUpdating || currentUserRole !== 'admin' || selectedRole === user.role}
            className={`flex-1 px-4 py-2 rounded-md font-medium transition-colors ${
              isUpdating || currentUserRole !== 'admin' || selectedRole === user.role
