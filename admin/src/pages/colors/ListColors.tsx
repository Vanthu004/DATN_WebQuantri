import { useEffect, useState } from "react";
import { getAllColors, softDeleteColor } from "../../services/color";
import { Color } from "../../interfaces/color";
import { useNavigate } from "react-router-dom";
import "../../css/color/listColor.css";

const ListColors = () => {
  const [colors, setColors] = useState<Color[]>([]);
  const [showDeleted, setShowDeleted] = useState(false);
  const [loading, setLoading] = useState(false);
  const [message, setMessage] = useState("");
  const [error, setError] = useState("");
  const navigate = useNavigate();

  useEffect(() => {
    fetchColors();
    // eslint-disable-next-line
  }, [showDeleted]);

  const fetchColors = async () => {
    setLoading(true);
    setMessage("");
    setError("");
    try {
      const data = await getAllColors();
      setColors(Array.isArray(data) ? data : []);
    } catch (error) {
      setColors([]);
      setError("Không thể tải danh sách màu!");
    } finally {
      setLoading(false);
    }
  };

  const handleDelete = async (id: string) => {
    if (!window.confirm("Bạn có chắc chắn muốn xóa màu này?")) return;
    setError("");
    setMessage("");
    try {
      await softDeleteColor(id);
      setMessage("Xóa màu thành công!");
      fetchColors();
    } catch (err) {
      setError("Xóa màu thất bại!");
    }
  };

  const filteredColors = colors.filter((color) => (showDeleted ? color.is_deleted : !color.is_deleted));

  return (
    <div className="w-full">
      <div className="flex items-center mb-4 gap-2">
        <button
          className={`option-btn ${!showDeleted ? "active" : ""}`}
          onClick={() => setShowDeleted(false)}
        >
          Màu chưa bị xóa
        </button>
        <button
          className={`option-btn ${showDeleted ? "active" : ""}`}
          onClick={() => setShowDeleted(true)}
        >
          Màu đã xóa
        </button>
        <button
          className="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded-lg shadow-md transition duration-300 ml-auto"
          onClick={() => navigate("/colors/add")}
        >
          Thêm màu
        </button>
      </div>
      {message && <div className="form-success mb-2">{message}</div>}
      {error && <div className="form-error mb-2">{error}</div>}
      <div className="color-table-scroll">
        <table className="min-w-max w-full bg-white rounded-lg shadow border border-gray-200">
          <thead>
            <tr className="bg-gray-100">
              <th className="px-4 py-2 border-b">#</th>
              <th className="px-4 py-2 border-b">Tên màu</th>
              <th className="px-4 py-2 border-b">Mã màu</th>
              <th className="px-4 py-2 border-b">Mã hex</th>
              <th className="px-4 py-2 border-b">Trạng thái</th>
              <th className="px-4 py-2 border-b">Đã xóa?</th>
              <th className="px-4 py-2 border-b">Ngày tạo</th>
              <th className="px-4 py-2 border-b">Ngày cập nhật</th>
              {!showDeleted && <th className="px-4 py-2 border-b">Hành động</th>}
            </tr>
          </thead>
          <tbody>
            {filteredColors.map((color, index) => (
              <tr key={color._id} className="hover:bg-gray-50">
                <td className="px-4 py-2 border-b">{index + 1}</td>
                <td className="px-4 py-2 border-b">{color.name}</td>
                <td className="px-4 py-2 border-b">{color.code || "--"}</td>
                <td className="px-4 py-2 border-b">
                  {color.hex_code ? (
                    <span className="inline-flex items-center gap-2">
                      <span
                        className="inline-block w-5 h-5 rounded-full border border-gray-300 align-middle"
                        style={{ background: color.hex_code }}
                        title={color.hex_code}
                      ></span>
                      {color.hex_code}
                    </span>
                  ) : (
                    "--"
                  )}
                </td>
                <td className="px-4 py-2 border-b">
                  <span className={`status-badge ${color.status}`}>{color.status}</span>
                </td>
                <td className="px-4 py-2 border-b">
                  {color.is_deleted ? (
                    <span className="deleted-badge">Đã xóa</span>
                  ) : (
                    <span className="not-deleted-badge">Chưa xóa</span>
                  )}
                </td>
                <td className="px-4 py-2 border-b">
                  {color.createdAt ? new Date(color.createdAt).toLocaleString() : ""}
                </td>
                <td className="px-4 py-2 border-b">
                  {color.updatedAt ? new Date(color.updatedAt).toLocaleString() : ""}
                </td>
                {!showDeleted && (
                  <td className="px-4 py-2 border-b">
                    <button
                      className="action-btn edit mr-2"
                      onClick={() => navigate(`/colors/update/${color._id}`)}
                    >
                      Sửa
                    </button>
                    <button
                      className="action-btn delete"
                      onClick={() => handleDelete(color._id)}
                    >
                      Xóa
                    </button>
                  </td>
                )}
              </tr>
            ))}
            {filteredColors.length === 0 && (
              <tr>
                <td colSpan={showDeleted ? 8 : 9} className="text-center py-4 text-gray-400">
                  {loading ? "Đang tải..." : "Không có dữ liệu"}
                </td>
              </tr>
            )}
          </tbody>
        </table>
      </div>
    </div>
  );
};

export default ListColors;
